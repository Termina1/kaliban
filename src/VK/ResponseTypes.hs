...
instance FromJSON a => FromJSON (APIResponse a) where
  parseJSON = withObject "result" $ \obj ->
    case (HM.lookup "error" obj) of
      Nothing -> case (HM.lookup "response" obj) of
                      Nothing -> fail $ "Invalid API response" ++ (show obj)
                      Just resp -> fmap APIResult (parseJSON resp)
      Just errParams -> withObject "error" (\errObject -> do
...
instance (FromJSON a, Show a) => FromJSON (WithCount a) where
  parseJSON = withObject "items with count" $ \obj -> do
    count <- obj .: "count"
    itemsp <- obj .: "items"
...
data LongpollResponse = LPErr Int | LPResult  (Int, [LongpollEvent])
  deriving (Show)
