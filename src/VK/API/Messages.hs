...
instance FromJSON ForwardedMessage where
  parseJSON = withObject "fwd_message" $ \o -> do
    fromId <- o .: "from_id"
    date <- o .: "date"
    text <- o .: "text"
...
getById owner ids preview_length = apiRequest owner "messages.getById" ([
    ("message_ids", toList ids), 
    ("preview_length", (show $ toInt preview_length))
  ] :: [(String, String)])
...
instance FromJSON LpServer where
  parseJSON = withObject "lp server" $ \o -> do
    key <- o .: "key"
    server <- o .: "server"
    ts <- o .: "ts"
    pts <- o .:? "pts" .!= Default
...
getLongpollServer owner pts lpVer = apiRequest owner "messages.getLongPollServer" ([
    ("need_pts", (show pts)),
    ("lp_version", (show lpVer))
  ] :: [(String, String)])
...
instance Paramable LpHistoryParams where
  toParams lp = let params = [("preview_length", show $ defaultTo 0 $ previewLength lp),
                              ...
                              ("onlines", fromBool $ defaultTo False $ onlines lp)] in
                                case (timestamp lp) of
...
instance FromJSON LpHistoryResult where
  parseJSON = withObject "lp response" $ \o -> do
    history <- o .: "history"
    messages <- o .: "messages"
    newPts <- o .: "new_pts"
